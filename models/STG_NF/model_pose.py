...
class FlowStep(nn.Module):
    def __init__(
        ...
        if A is None:
            ...
            self.A = torch.from_numpy(g.A).float().to(self.device)
    ...
    def forward(self, input, logdet=None, reverse=False, label=None):
        if not reverse:
            return self.normal_flow(input, logdet,self.flag)
        else:
            return self.reverse_flow(input, logdet)

    def normal_flow(self, input, logdet,flag):
        ...
        if self.flow_coupling == "additive":
            ...
            z2 = z2 + shift
            z2 = z2 * scale
            logdet = torch.sum(torch.log(scale), dim=[1, 2, 3]) + logdet ## 就应该这样计算，可以看推到公式
...
class STG_NF(nn.Module):
    def __init__(
        ...
        self.register_buffer( ### 就是收集 正常的姿态维度
            "prior_h",
            torch.zeros(
                [
                    1,
    ...
    def prior(self, data, label=None): ### 先验概率
        if data is not None:
            if label is not None:
                h = self.prior_h.repeat(data.shape[0], 1, 1, 1)
                h[label == 1] = self.prior_h_normal   ### 这句话是什么含义
        ...
        if self.learn_top:### 默认不执行
            h = self.learn_top_fn(h)
        #print(h[1])
        return split_feature(h, "split") #将特征向量按照通道分成两半，我没记错的话，通道只有两个，所以这里会减少一维度
## h 只有01 组成，怎么就成了平均值与方差了呢？
    ...
    def normal_flow(self, x, label, score): #### 这一个训练过程中用到了吗 label 全为1 是因为视频全都是正常的
        #print(x.shape,img_code.shape)
        #x = torch.cat((x,img_code),axis=1) ### 一定要体现出，这个姿势在这个背景下
        b, c, t, v = x.shape
        z, objective = self.flow(x, reverse=False) ### k层循环
        mean, logs = self.prior(x, label) # 这里用到了label，mean 是向量的前半段，log是特征向量的后半段？ 怎么理解
        ### 是不是说作者将原data的x数据集作为了均值和方差，有没有合理性？
        # print(mean.shape, logs.shape)
        objective += gaussian_likelihood(mean, logs, z)
        # img_mean, img_logs = self.prior(img, label)  # 这里用到了label,这里的公式是什么意思？
        # img_objective = gaussian_likelihood(img_mean, img_logs, img_code)
        # Full objective - converted to bits per dimension
        nll = (-objective) / (math.log(2.0) * c * t * v) #+ (-img_objective) / (math.log(2.0) * c * t *v)
