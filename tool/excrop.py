...
def json_to_img(json_path,img_path_root,save_path_root):
    ...
    name_list = json_path.split('.')[0].split('\\')[-1].split('_')

    scene_id = name_list[0]
    clip_id = name_list[1]
    save_path_file = r"C:\Users\molu\Desktop\2" #
    img_path_file =  r"D:\.mydatasets\ShanghaiTech\training\training\train_frame\08_001" #
        ...
        for frame in json_dict[person].keys():
            ...
            if not os.path.exists(img_path):
                with open('../error.txt', 'a', encoding="utf-8") as f:
                    f.write(img_path + "\t is not exist\n")
                continue
            crop_img = cut_person_img(img_path, json_xy, alpha=0.2)

            crop_img_path = os.path.join(person_path,name)
...
def main_deal(json_path):
    ## train
    json_to_img(r"/data/ShanghaiTech/pose/train/08_001_alphapose_tracked_person.json",
                r"D:\.mydatasets\ShanghaiTech\training\training\train_frame",
                r"D:\OneDrive\A-expriments\STG-NF-main\data\ShanghaiTech\crop\train")
    # for json in os.listdir(os.path.join(json_path,'train')):
    #     if json.endswith("person.json"):
    #         json_to_img(os.path.join(os.path.join(json_path,'train'),json), r"D:\.mydatasets\ShanghaiTech\training\training\train_frame", r"D:\OneDrive\A-expriments\STG-NF-main\data\ShanghaiTech\crop\train")
    # ## test:
...
main_deal(r"/data/ShanghaiTech/pose")
